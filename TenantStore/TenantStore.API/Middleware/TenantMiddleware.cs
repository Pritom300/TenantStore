...
public class TenantMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, ITenantProvider tenantProvider, ITenantService tenantService)
    {
        ...
        // PRIORITY 1: Check custom header (from Angular)
        if (context.Request.Headers.ContainsKey("X-Tenant-Subdomain"))
        {
            subdomain = context.Request.Headers["X-Tenant-Subdomain"].ToString();
        }

        // PRIORITY 2: Check hostname (for direct API access)
